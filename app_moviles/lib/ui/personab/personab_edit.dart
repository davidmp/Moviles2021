...
class PersonaBFormEdit extends StatefulWidget {
  ModeloPersona modelP;

  PersonaBFormEdit({this.modelP}): super();

  @override
  _PersonaBFormEditState createState() => _PersonaBFormEditState(modelP: modelP);
}
...
class _PersonaBFormEditState extends State<PersonaBFormEdit> {
  String _dni;
  String _nombre;
  String _telefono;
  int _edad;
  ...
  ModeloPersona modelP;
  _PersonaBFormEditState({this.modelP}):super();
  ...
  Widget _buildFechaNac() {
    return TextFormField(
      ...
      initialValue: modelP.fecha_nac,
      validator: (String value) {
        if (value.isEmpty) {
          return 'F. Nacimiento es campo Requerido';
        }
        if(!RegExp(r"^\d{4}[\-\/\s]?((((0[13578])|(1[02]))[\-\/\s]?(([0-2][0-9])|(3[01])))|(((0[469])|(11))[\-\/\s]?(([0-2][0-9])|(30)))|(02[\-\/\s]?[0-2][0-9]))$")
            .hasMatch(value)) {
